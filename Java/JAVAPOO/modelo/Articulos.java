...
public class Articulos {
    ...
    public boolean editarArticulo(String nombreNuevo, int stock, double precioUnitario, String nombreOriginal) {
        if (!buscarArticulo(nombreOriginal)) {
            return false;  // Artículo no encontrado
        }
        ...
        try {
            if (conexion != null) {
                String query = "UPDATE Articulos SET nombre=?, stock=?, precio_unitario=? WHERE nombre=?";
                psUpdate = conexion.prepareStatement(query);
                psUpdate.setString(1, nombreNuevo);
                psUpdate.setInt(2, stock);
                psUpdate.setDouble(3, precioUnitario);
                psUpdate.setString(4, nombreOriginal);
